...
## How to use
When in the jinja-sql syntax and in a .sql file, type 'dbt-' and be showered with snippets.
![set-default](./images/snippet-auto-complete.png)

## Features

### Generic Jinja
for loops, conditionals, blocks, variable settings etc. See [source](.vscode/snippets.code-snippets) for a full list
![jinja](./images/jinja_for_loop.gif)

### dbt-flavoured SQL
ref, source, log, config etc. See [source](.vscode/snippets.code-snippets) for a full list. 
...
### To associate your `.sql` files with `jinja-sql` language:

#### Set Default in Options Menu (CMD + SHIFT + P -> Change Language Mode -> Configure...)

![set-default](./images/default-language-setting.png)
